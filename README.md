...
当 Agent 开发或修改 Web 项目时，只进行关键路径冒烟测试，不默认执行完整回归：
...
1. 启动一次本地服务。
2. 打开主页面并确认页面能够加载。
3. 操作一个最重要、最能代表本次修改的功能。
4. 仅在需要时检查控制台或网络请求。
5. 只有界面布局发生变化时才截图，最多保留一张。

不需要逐个测试所有页面、按钮、路由和表单。需求分析 Agent 负责制定这条最小测试路径，开发与测试 Agent 执行一次，代码审查 Agent 只做一次针对修改点的验收，不重复完整测试。非 Web 任务跳过 Playwright。
